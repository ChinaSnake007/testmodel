...
class TransformerDiffusionBlock(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        combined_cond: torch.Tensor,  # 组合的时间和条件信息
    ) -> torch.Tensor:
        # Self attention
        x2 = self.norm1(x)
...
class TransformerDiffusion(nn.Module):
    ...
    def forward(
        ...
        t_emb = t_emb.unsqueeze(1).expand(-1, condition.size(1), -1)  # [B, seq_len, d_model]
        
        # 将时间信息和条件信息相加
        combined_cond = t_emb + condition  # [B, seq_len, d_model]
...
class NormalizedTrajectoryTransformerDiffusion(pl.LightningModule):
    ...
    def normalize_ids(self, ids):
        # 将ID归一化到[0,1]范围
        return ids.float() / (self.num_ids + 1)  # +1是为了考虑padding
    ...
    def forward(self, noisy_x, timesteps, masked_ids, mask=None):
        condition_embedding = self.id_embedding(masked_ids)  # [B, seq_len, d_model]
    ...
    def sample(self, masked_ids, mask=None):
...
def main():
    ...
    masked_ids = original_ids.clone()  # 在实际应用中，这里应该是你的条件ID
    ...
    for epoch in range(num_epochs):
        ...
        # 准备批次数据
        batch = (original_ids, masked_ids, None)
